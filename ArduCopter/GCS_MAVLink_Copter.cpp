...
void GCS_MAVLINK_Copter::handle_message_set_attitude_target(const mavlink_message_t &msg)
{
    ...
    // Drop out-of-order messages: if the sender packed a nonzero time_boot_ms,
    // reject any message whose timestamp is not newer than the last accepted one.
    // This prevents stale UDP packets from overwriting fresher commands.
    static uint32_t last_attitude_target_time_boot_ms;
    if (packet.time_boot_ms != 0) {
        if (packet.time_boot_ms < last_attitude_target_time_boot_ms) {
            return;
        }
        last_attitude_target_time_boot_ms = packet.time_boot_ms;
    }
    ...
    copter.mode_guided.set_angle(attitude_quat, ang_vel_body,
            climb_rate_ms_or_thrust, use_thrust, packet.time_boot_ms);
}
