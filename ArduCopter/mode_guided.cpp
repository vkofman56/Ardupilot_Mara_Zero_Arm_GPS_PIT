...
void ModeGuided::set_angle(const Quaternion &attitude_quat, const Vector3f &ang_vel_body, float climb_rate_ms_or_thrust, bool use_thrust, uint32_t sender_time_ms)
{
    ...
    guided_angle_state.update_time_ms = millis();
    guided_angle_state.sender_time_ms = sender_time_ms;
...
#if HAL_LOGGING_ENABLED
    // log target
    copter.Log_Write_Guided_Attitude_Target(guided_mode, roll_rad, pitch_rad, yaw_rad, ang_vel_body, guided_angle_state.thrust_norm, guided_angle_state.climb_rate_ms, guided_angle_state.sender_time_ms);
#endif
}
